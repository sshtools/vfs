	...
	protected FileSystem doCreateFileSystem(FileName fileName, FileSystemOptions fileSystemOptions) throws FileSystemException {
		...
		if (service == null) {
			...
			try {
				...
				String keyId = UserAuthenticatorUtils
				...
				if (keyId == null || key == null || keyId.length() + key.length() == 0) {
				...
				// logger.error(String.format("Failed to initialize Amazon S3
				// service client. Reason: %s",
