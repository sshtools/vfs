...
public class S3FileObject extends AbstractFileObject<S3FileSystem> {
		...
		if (!attached) {
			...
			if (!flaggedAsFolder) {
				try {
					...
					objectMetadata = getService().getObjectMetadata(bucket.getName(), candidateKey);
			...
			try {
				...
				candidateKey += FileName.SEPARATOR;
	 ...
	 * Other than <code>doListChildren</code> you could return FileObject's to e.g.
		...
		for (S3ObjectSummary summary : summaries) {
			if (!summary.getKey().equals(path)) {
				...
				if (childObject instanceof S3FileObject) {
					...
					childMetadata
							.setContentType(Mimetypes.getInstance().getMimetype(s3FileObject.getName().getBaseName()));
	...
	private AccessControlList getS3Acl() {
		String key = getS3Key();
		return "".equals(key) ? getService().getBucketAcl(getBucket().getName())
	 ...
	 *
	 * VFS interfaces doesn't provide interface to manage permissions. ACL can be
	 * accessed through {@link FileObject#getFileOperations()} Sample:
...
//				getBucket().getName(), getS3Key());

		if (StringUtils.isNullOrEmpty(getS3Key()))
			return String.format("s3://%s", getBucket().getName());
		else
			return String.format("s3://%s/%s", getBucket().getName(), getS3Key());
	}
