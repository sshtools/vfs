...
public class S3FileObject extends AbstractFileObject {
	...
	public S3FileObject(AbstractFileName fileName, S3FileSystem fileSystem,
			AmazonS3Client service, Bucket bucket) throws FileSystemException {
	...
	protected OutputStream doGetOutputStream(boolean bAppend) throws Exception {
		return new S3OutputStream(
				Channels.newOutputStream(getCacheFileChannel()), this.service,
				this.object);
	}
	...
	private boolean isImmediateDescendant(final String parent,
			final String child) {
		if (!child.startsWith(parent)) {
	...
	private void downloadOnce() throws FileSystemException {
		if (!this.downloaded) {
			...
			try {
				S3Object obj = this.service.getObject(this.bucket.getName(),
						getS3Key());
				this.logger.info(String.format("Downloading S3 Object: %s",
						objectPath));
	...
	private AccessControlList getS3Acl() throws AmazonServiceException {
		String key = getS3Key();
		return "".equals(key) ? this.service
				.getBucketAcl(this.bucket.getName()) : this.service
				.getObjectAcl(this.bucket.getName(), key);
	}
	...
	private void putS3Acl(AccessControlList s3Acl) throws Exception {
		...
		if ("".equals(key)) {
			...
			this.service.setObjectAcl(this.bucket.getName(),
					this.object.getKey(), s3Acl);
		}
	}
	...
	public Acl getAcl() throws FileSystemException {
		...
		for (Grant item : s3Acl.getGrants()) {
			...
			if (perm.equals(Permission.FullControl)) {
				...
				this.logger.error(String.format("Skip unknown permission %s",
						perm));
				continue;
			}
		...
		while (keys.hasMoreElements()) {
			...
			if (Arrays.equals(rights, allRights)) {
				...
				this.logger.error(String.format(
						"Skip unknown set of rights %s", rights.toString()));
				continue;
			}
			...
			if (group.equals(Acl.Group.EVERYONE)) {
				...
				this.logger
						.error(String.format("Skip unknown group %s", group));
				continue;
			}
	...
	private class S3OutputStream extends MonitorOutputStream {
		...
		public S3OutputStream(OutputStream out, AmazonS3Client service,
				S3Object object) {
			super(out);
			this.service = service;
			this.object = object;
		...
		protected void onClose() throws IOException {
			this.object.getObjectMetadata().setContentType(
					Mimetypes.getInstance().getMimetype(this.object.getKey()));
			try {
				this.service.putObject(this.object.getBucketName(),
						this.object.getKey(),
						Channels.newInputStream(getCacheFileChannel()),
						this.object.getObjectMetadata());
			} catch (AmazonServiceException e) {
