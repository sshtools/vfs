...
public class RFBFTPFileObject extends AbstractFileObject {
	...
	protected RFBFTPFileObject(final AbstractFileName name, final RFBFTPFileSystem fileSystem) {
	...
	protected void doRename(FileObject newfile) throws Exception {
		...
		if (oldName.equals(newName)) {
			throw new FileSystemException("vfs.provider.rfb/rename-identical-files", new Object[] { newName });
		}
	...
	protected String[] doListChildren() throws Exception {
		...
		for (int i = 0; i < array.length; i++) {
			if (!array[i].getName().equals(".") && !array[i].getName().equals("..")) {
