...
import com.sshtools.vfs.ldap.DefaultLDAPClient.ResultMapper;

public class SearchResultsIterator<T extends Object> implements Iterator<T> {
	final static Log LOG = LogFactory.getLog(SearchResultsIterator.class);
	...
	public SearchResultsIterator(Collection<? extends Name> dns, Filter filter, SearchControls searchControls,
			ResultMapper<T> filteredMapper, LdapContext context, FileSystemOptions opts) {
					...
					try {
						...
						if (cookie != null) {
							context.setRequestControls(new Control[] { new PagedResultsControl(
									LDAPFileSystemConfigBuilder.getInstance().getMaxPageSize(opts), cookie, Control.CRITICAL) });
						} else {
							context.setRequestControls(new Control[] { new PagedResultsControl(
									LDAPFileSystemConfigBuilder.getInstance().getMaxPageSize(opts), Control.CRITICAL) });
						}
						listIterator = context.search(dn, filter.encode(), searchControls);
					} catch (PartialResultException e) {
						if (LDAPFileSystemConfigBuilder.getInstance().isFollowReferrals(opts)) {
					...
					try {
						...
						while (next == null && listIterator.hasMoreElements()) {
							...
							if (filteredMapper.isApplyFilters()) {
								...
								boolean include = LDAPFileSystemConfigBuilder.getInstance().getIncludes(opts).isEmpty();
								if (!include) {
									for (Name name : LDAPFileSystemConfigBuilder.getInstance().getIncludes(opts)) {
								...
								for (Name name : LDAPFileSystemConfigBuilder.getInstance().getExcludes(opts)) {
									if (resultName.startsWith(name)) {
										include = false;
										break;
									}
