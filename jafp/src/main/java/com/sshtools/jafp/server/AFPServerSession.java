...
import com.sshtools.jafp.common.AFPVolumeInfo;
 ...
 */
final class AFPServerSession extends Utility implements AFPConstants {
	...
	// ----------------------------------------------------------------------------------------
	AFPServerSession(AFPServer server, Socket socket) {
		this.server = server;
		this.socket = socket;
	...
	private void error(boolean value, String msg) throws Exception {
		if (value) {
	...
	private DSI_Packet recvPacket() throws IOException {
		DSI_Packet dp = null;
	...
	private void sendPacket(DSI_Packet dp) throws IOException {
		synchronized (output) {
			dp.write(output);
		}
	...
	private abstract class CommandQueue extends Queue implements Runnable {
		...
		public abstract void handleCommand(DSI_Packet dp) throws IOException;
	}
				...
				while (valid) {
					...
					case DSI_Constants.CMD_OPEN_SESSION:
						...
						case DSI_Constants.OPT_ATTN_QUANT:
							...
							ww.writeInt(maxPacket); // server quantum (max
													// client request)
							sendPacket(dp);
							break;
						// sent by a server
	...
	private class CommandLoop extends CommandQueue {
		public void handleCommand(DSI_Packet dp) throws IOException {
			ByteReader rr = dp.getReader();
			ByteWriter ww = dp.getWriter();
			int err = ERR_NO_ERR;
			int cmd = rr.readUnsignedByte();
			try {
				...
				case CMD_LOGIN:
					...
					break;
				case CMD_LOGIN_EXT:
					err = cmdLoginExt(rr, ww);
					break;
				case CMD_LOGIN_CONT:
					err = cmdLoginCont(rr, ww);
					break;
				case CMD_LOGOUT:
					err = cmdLogout(rr, ww);
					break;
				case CMD_MAP_ID:
					...
					break;
				case CMD_CREATE_DIR:
					err = cmdCreateDir(rr, ww);
					break;
				case CMD_CREATE_FILE:
					err = cmdCreateFile(rr, ww);
					break;
				case CMD_CREATE_ID:
					err = ERR_PARAM_ERR;
					break; // TODO
				case CMD_GET_USER_INFO:
					err = cmdGetUserInfo(rr, ww);
					break;
				case CMD_GET_SRVR_PARMS:
					err = cmdGetServerParams(rr, ww);
					break;
				case CMD_GET_VOL_PARMS:
					err = cmdGetVolumeParams(rr, ww);
					break;
				case CMD_GET_FORK_PARMS:
					err = cmdGetForkParams(rr, ww);
					break; // TODO
				case CMD_GET_FILE_DIR_PARMS:
					err = cmdGetFileDirParams(rr, ww);
					break;
				case CMD_GET_SESSION_TOKEN:
					err = cmdGetSessionToken(rr, ww);
					break;
				case CMD_SET_VOL_PARMS:
					err = ERR_NO_ERR;
					break; // TODO
				case CMD_SET_DIR_PARMS:
					err = cmdSetDirParams(rr, ww);
					break;
				case CMD_SET_FORK_PARMS:
					err = cmdSetForkParams(rr, ww);
					break;
				case CMD_SET_FILE_PARMS:
					err = cmdSetFileParams(rr, ww);
					break;
				case CMD_SET_FILE_DIR_PARMS:
					err = cmdSetFileDirParams(rr, ww);
					break;
				case CMD_OPEN_DT:
					err = ERR_MISC_ERR;
					break; // TODO
				case CMD_OPEN_VOL:
					err = cmdOpenVolume(rr, ww);
					break;
				case CMD_OPEN_DIR:
					err = cmdOpenDir(rr, ww);
					break;
				case CMD_OPEN_FORK:
					err = cmdOpenFork(rr, ww);
					break;
				case CMD_BYTE_RANGE_LOCK:
					err = cmdByteRangeLock(rr, ww);
					break;
				case CMD_BYTE_RANGE_LOCK_EXT:
					err = cmdByteRangeLockExt(rr, ww);
					break;
				case CMD_ENUMERATE:
					err = cmdEnumerate(rr, ww);
					break;
				case CMD_ENUMERATE_EXT2:
					err = cmdEnumerateExt2(rr, ww);
					break;
				case CMD_READ:
					err = cmdRead(rr, ww);
					break;
				case CMD_READ_EXT:
					err = cmdReadExt(rr, ww);
					break;
				case CMD_WRITE:
					err = cmdWrite(rr, ww);
					break;
				case CMD_WRITE_EXT:
					err = cmdWriteExt(rr, ww);
					break;
				case CMD_FLUSH_FORK:
					err = cmdFlushFork(rr, ww);
					break;
				case CMD_CLOSE_VOL:
					err = cmdCloseVolume(rr, ww);
					break;
				case CMD_CLOSE_DIR:
					err = cmdCloseDir(rr, ww);
					break;
				case CMD_CLOSE_FORK:
					err = cmdCloseFork(rr, ww);
					break;
				case CMD_MOVE_AND_RENAME:
					err = cmdMoveAndRename(rr, ww);
					break;
				case CMD_RENAME:
					err = cmdRename(rr, ww);
					break;
	...
	// CMD_LOGIN
	private int cmdLogin(ByteReader rr, ByteWriter ww) throws IOException {
	...
	// CMD_LOGIN_EXT
	private int cmdLoginExt(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		...
		debug("loginext afp=" + afp + ",uam=" + uam + ",user=" + usName + ",path=" + paName + ",ssn=" + getSessionID() + ",flags="
				+ flags);
		return loginCommon(uam, usName, rr, ww);
	}

	// login common helper method
	private int loginCommon(String uam, String user, ByteReader rr, ByteWriter ww) throws IOException {
		...
		case UAM_GUEST:
			...
			if (user != null && server.hasUser(user)) {
				...
				if (user == null)
					LOG.warn(String.format("Client requested guest authentication but this server has no guest user."));
				else
					LOG.warn(String.format("Server could not find guest user %s", user));
		...
		case UAM_CLEARTEXT:
			...
			if (!server.checkPassword(user, pass)) {
				LOG.warn(String.format("User authentication for %s failed.", user));
				return ERR_USER_NOT_AUTH;
			}
			userName = user;
		...
		case UAM_DHX_128:
			...
			byte[] sessionKeyBytes = keyBytes(sessionKey);
			new CAST128(sessionKeyBytes).encrypt(inbuf, 0, outbuf, 0, 32, AFPConstants.DHX_S2CIV);
	...
	// CMD_LOGIN_CONT
	private int cmdLoginCont(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		...
		case UAM_DHX_128:
			...
			c.decrypt(inbuf, 0, outbuf, 0, 32, AFPConstants.DHX_C2SIV);
	...
	// CMD_LOGOUT
	private int cmdLogout(ByteReader rr, ByteWriter ww) throws IOException {
		return ERR_NO_ERR;
	}

	// CMD_GET_USER_INFO
	private int cmdGetUserInfo(ByteReader rr, ByteWriter ww) throws IOException {
		...
		if (hasBits(flags, 0x1)) {
			ww.writeInt(0x00000000);
		} // user = guest
		if (hasBits(flags, 0x2)) {
			ww.writeInt(0x00000000);
		} // group = guest
		return ERR_NO_ERR;
	}
	...
	// CMD_GET_SRVR_PARMS
	private int cmdGetServerParams(ByteReader rr, ByteWriter ww) throws IOException {
		AFPServerVolume v[] = server.getVolumes();
		AFPServerParams p = new AFPServerParams(v);
		p.write(ww);
		return ERR_NO_ERR;
	}

	// CMD_OPEN_VOL
	private int cmdOpenVolume(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int flags = rr.readUnsignedShort();
		...
		AFPServerVolume vol = server.getVolume(volName);
		if (vol == null) {
	...
	// CMD_DELETE
	private int cmdDelete(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		...
		AFPCNode node = openPath(volID, dirID, pathName).getNode();
	...
	private int cmdGetFileDirParams(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
		...
		if (node.isDirectory()) {
			sendDirectoryInfo(ww, node, dirFlags, AFPConstants.MODE_EXT);
		} else {
			sendFileInfo(ww, node, fileFlags, AFPConstants.MODE_EXT);
		}
		return ERR_NO_ERR;
	}

	// CMD_GET_FORK_PARMS (AFP2.x only)
	private int cmdGetForkParams(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
	...
	private int cmdSetForkParams(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int forkRef = rr.readUnsignedShort();
		int flags = rr.readUnsignedShort();
	...
	// CMD_SET_FILE_PARMS
	private int cmdSetFileParams(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
		int fileFlags = rr.readUnsignedShort();
	...
	// CMD_SET_DIR_PARMS
	private int cmdSetDirParams(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
		int dirFlags = rr.readUnsignedShort();
	...
	private int cmdSetFileDirParams(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
		int flags = rr.readUnsignedShort();
	...
	// CMD_GET_VOL_PARMS
	private int cmdGetVolumeParams(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int flags = rr.readUnsignedShort();
		AFPServerVolume vol = server.getVolume(volID);
		if (vol == null) {
			return ERR_PARAM_ERR;
		}
		sendVolumeInfo(ww, vol, flags);
	...
	private int cmdGetSessionToken(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
	...
	// CMD_CLOSE_VOL
	private int cmdCloseVolume(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		return ERR_NO_ERR;
	}
	...
	// CMD_MAP_ID
	private int cmdMapID(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
	...
	// CMD_CREATE_DIR
	private int cmdCreateDir(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
		String pathName = rr.readTypedString();
	...
	// CMD_CREATE_FILE
	private int cmdCreateFile(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
		String pathName = rr.readTypedString();
	...
	// CMD_OPEN_DIR
	private int cmdOpenDir(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
		String pathName = rr.readTypedString();
		debug("opendir vol=" + hex(volID) + ",dir=" + hex(dirID) + ",path=" + pathName);
		AFPCNode node = openPath(volID, dirID, pathName).open().getNode();
		if (!node.isDirectory()) {
	...
	// CMD_OPEN_FORK
	private int cmdOpenFork(ByteReader rr, ByteWriter ww) throws IOException {
		int which = rr.readUnsignedByte(); // bits: 7=fork (0=file, 1=resource)
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
		int flags = rr.readUnsignedShort(); // bits: getFileParams bits
		int mode = rr.readUnsignedShort(); // bits: 0=read, 1=write, 4=readlock,
											// 5=writelock
		String pathName = rr.readTypedString();
		...
		int fid = nextForkID();
		AFPFork fork = hasBits(which, 0x80) ? node.openResourceFork(mode) : node.openFileFork(mode);
		if (fork == null) {
			return ERR_ACCESS_DENIED;
		}
	...
	// CMD_RENAME
	private int cmdRename(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
	...
	// CMD_MOVE_AND_RENAME
	private int cmdMoveAndRename(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		...
		String newName = rr.readTypedString();
		debug("move/rename vol=" + volID + ",srcDir=" + srcDirID + ",dstDir=" + dstDirID + ",srcPath=" + srcPath + ",dstPath="
				+ dstPath + ",name=" + newName);
	...
	// CMD_BYTE_RANGE_LOCK
	private int cmdByteRangeLock(ByteReader rr, ByteWriter ww) throws IOException {
	...
	private int cmdByteRangeLockExt(ByteReader rr, ByteWriter ww) throws IOException {
		// bits: 0=mode (0=lock, 1=unlock)
		// bits: 7=relative to (0=from start, 1=from end)
		int flags = rr.readUnsignedByte();
		int forkRef = rr.readUnsignedShort();
	...
	// CMD_READ
	private int cmdRead(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int forkRef = rr.readUnsignedShort();
		long offset = rr.readInt();
		long length = rr.readInt();
		...
		debug("read fork=" + hex(forkRef) + ",off=" + hex(offset) + ",len=" + hex(length) + ",nlm=" + hex(nlMask) + ",nlc="
				+ hex(nlChar));
	...
	// CMD_READ_EXT
	private int cmdReadExt(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int forkRef = rr.readUnsignedShort();
		long offset = rr.readLong();
		long length = rr.readLong();
	...
	// CMD_WRITE
	private int cmdWrite(ByteReader rr, ByteWriter ww) throws IOException {
		int flag = rr.readUnsignedByte(); // bits: 7=relative to (0=from start,
											// 1=from end)
		int forkRef = rr.readUnsignedShort();
		long offset = rr.readInt();
		long length = rr.readInt();
	...
	// CMD_WRITE_EXT
	private int cmdWriteExt(ByteReader rr, ByteWriter ww) throws IOException {
		int flag = rr.readUnsignedByte(); // bits: 7=relative to (0=from start,
											// 1=from end)
		int forkRef = rr.readUnsignedShort();
		long offset = rr.readLong();
	...
	// CMD_FLUSH_FORK
	private int cmdFlushFork(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int forkRef = rr.readUnsignedShort();
	...
	// CMD_CLOSE_DIR
	private int cmdCloseDir(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
	...
	// CMD_CLOSE_FORK
	private int cmdCloseFork(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int forkRef = rr.readUnsignedShort();
		AFPFork fork = getFork(forkRef);
	...
	// CMD_ENUMERATE
	private int cmdEnumerate(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
		int fileFlags = rr.readUnsignedShort();
		...
		String pathName = rr.readTypedString();
		debug("enum vol=" + volID + ",dir=" + dirID + ",ff=" + hex(fileFlags) + ",df=" + hex(dirFlags) + ",xrec=" + hex(maxRecords)
				+ ",idx=" + hex(startIndex) + ",xrep=" + hex(maxReply) + ",path=" + pathName);
		AFPCNode node = openPath(volID, dirID, pathName).getNode();
		ww.writeShort(fileFlags);
		ww.writeShort(dirFlags);
		Iterator<AFPCNode> en = node.getChildren();
		if (en == null) {
			return ERR_MISC_ERR;
		}
		if (!en.hasNext()) {
		...
		for (int i = 0; sent < maxRecords && en.hasNext(); i++) {
			...
			ByteWriter w2 = new ByteWriter(2048);
			if (next.isDirectory()) {
				sendDirectoryInfo(w2, next, dirFlags, AFPConstants.MODE_OLD);
			} else {
				sendFileInfo(w2, next, fileFlags, AFPConstants.MODE_OLD);
			}
	...
	// TODO: uses too many byte writers. optimize byte writers and re-use.
	// CMD_ENUMERATE_EXT2
	private int cmdEnumerateExt2(ByteReader rr, ByteWriter ww) throws IOException {
		rr.skip(1);
		int volID = rr.readUnsignedShort();
		int dirID = rr.readInt();
		int fileFlags = rr.readUnsignedShort();
		...
		for (int i = 0; sent < maxRecords && en.hasNext(); i++) {
			...
			if (next.isDirectory()) {
				sendDirectoryInfo(w2, next, dirFlags, AFPConstants.MODE_EXT);
			} else {
				sendFileInfo(w2, next, fileFlags, AFPConstants.MODE_EXT);
			}
			byte data[] = w2.toByteArray();
			if (data.length % 2 == 0) {
	...
	private void sendVolumeInfo(ByteWriter ww, AFPServerVolume vol, int flags) throws IOException {
		new AFPVolumeInfo(flags, vol).write(ww);
	}

	// USED BY: GetFileDirParams
	private void sendDirectoryInfo(ByteWriter ww, AFPCNode node, int flags, int mode) throws IOException {// 0x80
																											// =
																											// directory
																											// +
																											// 0x00
																											...
																											// pad
		switch (mode) {
		case AFPConstants.MODE_OLD:
			ww.writeByte(0x80);
			break;
		case AFPConstants.MODE_EXT:
			ww.writeShort(0x8000);
			break;
		}
		new AFPDirectoryInfo(flags, node).write(ww);
	}

	// USED BY: SetDirParams
	private void recvDirectoryInfo(ByteReader rr, AFPCNode node, int flags) throws IOException {
		rr.skipBytes(rr.getPosition() % 2);
		if (hasBits(flags, DIR_BIT_ATTRIBUTE)) {
			node.setAttributes(rr.readShort());
		}
		if (hasBits(flags, DIR_BIT_CREATE_DATE)) {
			node.setCreateDate(rr.readInt());
		}
		if (hasBits(flags, DIR_BIT_MOD_DATE)) {
			node.setModifiedDate(rr.readInt());
		}
		if (hasBits(flags, DIR_BIT_BACKUP_DATE)) {
			node.setBackupDate(rr.readInt());
		}
		if (hasBits(flags, DIR_BIT_FINDER_INFO)) {
			node.setFinderInfo(rr.readBytes(32));
		}
		if (hasBits(flags, DIR_BIT_UNIX_PRIVS)) {
			node.setUnixPrivs(rr.readBytes(16));
		}
		if (hasBits(flags, DIR_BIT_PARENT_DIR_ID | DIR_BIT_LONG_NAME | DIR_BIT_SHORT_NAME | DIR_BIT_NODE_ID
				| DIR_BIT_OFFSPRING_COUNT | DIR_BIT_OWNER_ID | DIR_BIT_GROUP_ID | DIR_BIT_ACCESS_RIGHTS | DIR_BIT_UTF8_NAME)) {
			throw new AFPError(ERR_BITMAP_ERR);
		}
	}

	// USED BY: GetFileDirParams, EnumerateExt2
	private void sendFileInfo(ByteWriter ww, AFPCNode node, int flags, int mode) throws IOException {
		// 0x00 = file + 0x00 = pad
		switch (mode) {
		case AFPConstants.MODE_OLD:
			ww.writeByte(0x00);
			break;
		case AFPConstants.MODE_EXT:
			ww.writeShort(0x0000);
			break;
		}
		new AFPFileInfo(flags, node).write(ww);
	}

	// USED BY: SetFileParams
	private void recvFileInfo(ByteReader rr, AFPCNode node, int flags) throws IOException {
		rr.skipBytes(rr.getPosition() % 2);
		if (hasBits(flags, FILE_BIT_ATTRIBUTE)) {
			node.setAttributes(rr.readShort());
		}
		if (hasBits(flags, FILE_BIT_CREATE_DATE)) {
			node.setCreateDate(rr.readInt());
		}
		if (hasBits(flags, FILE_BIT_MOD_DATE)) {
			node.setModifiedDate(rr.readInt());
		}
		if (hasBits(flags, FILE_BIT_BACKUP_DATE)) {
			node.setBackupDate(rr.readInt());
		}
		if (hasBits(flags, FILE_BIT_FINDER_INFO)) {
			node.setFinderInfo(rr.readBytes(32));
		}
		if (hasBits(flags, FILE_BIT_UNIX_PRIVS)) {
			node.setUnixPrivs(rr.readBytes(16));
		}
		if (hasBits(flags,
				FILE_BIT_PARENT_DIR_ID | FILE_BIT_LONG_NAME | FILE_BIT_SHORT_NAME | FILE_BIT_NODE_ID | FILE_BIT_DATA_FORK_LEN
						| FILE_BIT_RSRC_FORK_LEN | FILE_BIT_XDATA_FORK_LEN | FILE_BIT_LAUNCH_LIMIT | FILE_BIT_UTF8_NAME
						| FILE_BIT_XRSRC_FORK_LEN)) {
			throw new AFPError(ERR_BITMAP_ERR);
		}
	}

	private void print(String t, int v) {
		debug("(" + t + ") = (" + v + ")");
	}

	private void print(String t, long v) {
		debug("(" + t + ") = (" + v + ")");
	}
	...
	private Path openPath(int volID, int dirID, String pathName) throws AFPError {
		return new Path(volID, dirID, pathName).open();
	}

	private Path createDirPath(int volID, int dirID, String pathName) throws AFPError {
		return new Path(volID, dirID, pathName).createDir();
	}

	private Path createFilePath(int volID, int dirID, String pathName) throws AFPError {
	...
	private class Path {
		...
		Path(int volID, int dirID, String pathName) throws AFPError {
			...
			if (node == null) {
				throw new AFPError(ERR_OBJECT_NOT_FOUND);
			}
			path = pathName;
		}
		...
		Path create(boolean dir) throws AFPError {
			if (path == null || path.length() == 0) {
				throw new AFPError(ERR_OBJECT_EXISTS);
			}
			AFPCNode nnode = dir ? node.createDirectory(path) : node.createFile(path);
			if (nnode == null) {
				throw new AFPError(ERR_ACCESS_DENIED);
			}
			node = nnode;
			return this;
		}
	...
	private static int getUAM(String uam) {
		...
		if (uam.equals(UAM_STR_GUEST)) {
			return UAM_GUEST;
		}
		if (uam.equals(UAM_STR_CLEARTEXT)) {
			return UAM_CLEARTEXT;
		}
		if (uam.equals(UAM_STR_RANDOM_NUM1)) {
			return UAM_RANDOM_NUM1;
		}
		if (uam.equals(UAM_STR_RANDOM_NUM2)) {
			return UAM_RANDOM_NUM2;
		}
		if (uam.equals(UAM_STR_DHX_128)) {
			return UAM_DHX_128;
		}
		if (uam.equals(UAM_STR_DHX_DYNAMIC)) {
			return UAM_DHX_DYNAMIC;
		}
		if (uam.equals(UAM_STR_KERBEROS)) {
			return UAM_KERBEROS;
		}
		return UAM_UNKNOWN;
	}
