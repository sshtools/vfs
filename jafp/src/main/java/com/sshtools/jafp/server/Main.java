...
public class Main extends Utility implements AFPConstants, DSI_Constants
{
		...
		throws Exception
	{
		...
		for (Enumeration e = volumes.keys(); e.hasMoreElements(); )
		{
			...
			int port = vol.intValue("port", AFPConstants.TCP_PORT);
		...
		for (Enumeration e = ports.keys(); e.hasMoreElements(); )
		{
			...
			for (Enumeration v = port.keys(); v.hasMoreElements(); )
			{
				...
				if (!(nv instanceof AFPServerVolume))
				{
					continue;
				}
				server.addVolume((AFPServerVolume)nv);
			}
		...
		throws Exception
	{
		...
		AFPServerInfo si = new AFPServerInfo(dp.getReader());
