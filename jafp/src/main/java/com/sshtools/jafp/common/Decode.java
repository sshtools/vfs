...
class Decode extends Utility implements AFPConstants, DSI_Constants {
	...
	public static void afpRequest(byte b[], int cmd) throws IOException {
		switch (cmd) {
		case CMD_LOGIN:
			raw(b, new Object[][] { { I_BYTE, "Command" }, { I_PSTRING, "AFPVersion" }, { I_PSTRING, "UAM" }, });
			break;
		case CMD_LOGIN_CONT:
			raw(b, new Object[][] { { I_BYTE, "Command" }, { I_BYTE, "Pad" }, { I_SHORT, "ID" }, });
			break;
		case CMD_LOGIN_EXT:
			raw(b, new Object[][] { { I_BYTE, "Command" }, { I_BYTE, "Pad" }, { I_SHORT, "Flags" }, { I_PSTRING, "AFPVersion" },
					{ I_PSTRING, "UAM" }, { I_TYPEDSTRING, "UserName" }, { I_TYPEDSTRING, "PathName" }, });
			break;
		case CMD_GET_USER_INFO:
			raw(b, new Object[][] { { I_BYTE, "Command" }, { I_BYTE, "Pad" }, { I_INTEGER, "UserID", }, { I_SHORT, "Bitmap", }, });
			break;
		case CMD_OPEN_VOL:
			raw(b, new Object[][] { { I_BYTE, "Command" }, { I_BYTE, "Pad" }, { I_SHORT, "Bitmap", },
					{ I_PSTRING, "VolumeName" }, });
			break;
		case CMD_GET_FILE_DIR_PARMS:
			raw(b, new Object[][] { { I_BYTE, "Command" }, { I_BYTE, "Pad" }, { I_SHORT, "VolumeID" }, { I_INTEGER, "DirectoryID" },
					{ I_SHORT, "FileBitmap" }, { I_SHORT, "DirBitmap" }, { I_TYPEDSTRING, "PathName" }, });
			break;
		}
	}

	public static void raw(byte b[], Object rule[][]) throws IOException {
