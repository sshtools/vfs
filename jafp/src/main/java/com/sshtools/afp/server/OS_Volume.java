...
class OS_Volume extends AFPServerVolume
{
	private final static int LOCK_BITS = AFPCNode.MODE_WRITE_LOCK | AFPCNode.MODE_READ_LOCK;
	...
	@Override
	public AFPCNode getCNode(int id)
	{
		return getNode(id);
	}
	...
	{
		if (Utility.hasBits(flags, AFPCNode.MODE_WRITE) && !readonly)
		{
			return "rw";
		}
		else
	...
	private class VolumeNode extends OSNode
	{
		...
		public AFPCNode getChild(String name)
		{
	...
	private class OSNode extends AFPCNode
	{
		private int pid;
		private File file;
		...
		*/
		...
		private boolean canOpen(boolean data, File file, int flags)
		{
			...
			if (Utility.hasBits(flags, AFPCNode.MODE_WRITE))
			{
		...
		@Override
		public synchronized boolean moveTo(AFPCNode dir, String name)
		{
			if (!dir.isDirectory())
			{
				return false;
		...
		@Override
		public AFPCNode createDirectory(String name)
		{
			AFPCNode ndir = getChild(name);
			if (ndir != null)
			{
				return null;
			}
		...
		@Override
		public AFPCNode createFile(String name)
		{
			AFPCNode ndir = getChild(name);
			if (ndir != null)
			{
		...
		public Iterator<AFPCNode> getChildren()
		{
			try {
				...
				throw new IllegalStateException(String.format("Could not list children of %s", file), e);
			}
		}
		...
		@Override
		public AFPFork openFileFork(int flags)
		{
		...
		@Override
		public AFPFork openResourceFork(int flags)
		{
		...
		private class NodeIterator implements Iterator<AFPCNode>
		{
			...
			NodeIterator(File dir) throws FileNotFoundException
			{
				this.files = dir.listFiles();
				if(this.files == null)
					throw new FileNotFoundException("Could not list " + dir);
				findNext();
			}
		...
		private class DataFork extends AFPFork
		{
