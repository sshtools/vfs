...
public class DSI_Packet extends Utility implements DSI_Constants {
	...
	public DSI_Packet(int bufsize) {
		payload = new byte[bufsize + 128];
		header = new byte[16];
		writer = new ByteWriter(payload);
	}

	public DSI_Packet(InputStream is) throws IOException {
		this(0x8000);
		read(is);
	}

	public DSI_Packet(int f, int c, int id, byte d[]) throws IOException {
		this(d.length);
		flags = f;
		command = c;
		requestID = id;
	...
	private void readData(InputStream is, byte b[], int len) throws IOException {
		if (len > b.length) {
	...
	public void read(InputStream is) throws IOException {
		...
		// if (dataLength > maxR) { maxR = dataLength; System.out.println("max
		// read now "+maxR); }
	}
	...
	public void write(OutputStream os) throws IOException {
		...
		os.flush();
		// if (dataLength > maxW) { maxW = dataLength; System.out.println("max
		// write now "+maxW); }
	}
	...
	public String toString() {
		if (command == CMD_COMMAND) {
			...
			if (cmd >= AFPConstants.COMMAND.length || cmd < 0) {
				cmd = 0;
			}
			return "f=" + hex(flags) + ",c=" + hex(command) + ",id=" + hex(requestID) + ",edo=" + hex(errDataOff) + ",rd="
					+ hex(dataLength) + ",sd=" + hex(writer.getSize()) + ",cmd=" + AFPConstants.COMMAND[cmd];
		} else {
			return "f=" + hex(flags) + ",c=" + hex(command) + ",id=" + hex(requestID) + ",edo=" + hex(errDataOff) + ",rl="
					+ hex(dataLength) + ",sd=" + hex(writer.getSize()) + ",dsi=" + COMMAND[command];
		}
	}
}
