...
public final class ByteReader extends Utility {
	...
	public ByteReader(byte b[], int max) {
		this.max = max;
		this.data = b;
		this.pos = 0;
	}

	public long writeToFile(DataOutput file, long length) throws IOException {
	...
	public String readPString(int pos) throws IOException {
		int opos = this.pos;
		seek(pos);
		try {
			return readPString();
	...
	public String readAFPStringDeferred() throws IOException {
		int newpos = readUnsignedShort() + deferredOffset;
		return readAFPString(newpos);
	}
	
	public String readPStringDeferred() throws IOException {
		int newpos = readUnsignedShort() + deferredOffset;
		return readPString(newpos);
	}

	public String readAFPString(int pos) throws IOException {
		int opos = this.pos;
		seek(pos);
		try {
			return readAFPString();
	...
	public int readInt() {
		return (int) (((data[pos++] & 0xff) << 24) | ((data[pos++] & 0xff) << 16) | ((data[pos++] & 0xff) << 8)
				| (data[pos++] & 0xff));
	}

	public long readLong() {
		return (long) (((long) (data[pos++] & 0xff) << 56) | ((long) (data[pos++] & 0xff) << 48)
				| ((long) (data[pos++] & 0xff) << 40) | ((long) (data[pos++] & 0xff) << 32) | ((long) (data[pos++] & 0xff) << 24)
				| ((long) (data[pos++] & 0xff) << 16) | ((long) (data[pos++] & 0xff) << 8) | (long) (data[pos++] & 0xff));
	}
