...
//	private final static int TYPE_AFPSTRING_ARR = 4;
	...
	public void readFromInput(InputStream in, int len) throws IOException {
		int read = 0;
		while (read < len) {
	...
	public void readFromInput(DataInput file, int length) throws IOException {
	...
	private void writeDeferred(Object s, int type) {
		if (deferred == null) {
			deferred = new Vector<Deferred>(3);
		}
	...
	public void writeTypedPString(String s) {
		write(3);
		writeAFPString(s);
		
	}
	
	public void writeTypedAFPString(String s) {
		write(0);
		writeAFPString(s);
	}

	public void writeCString(String s, int sz) {
		byte[] a = s.getBytes();
		byte[] b = new byte[Math.min(sz, a.length + 1)];
	...
	private class Deferred {
		...
		void writeString() {
			...
			switch (type) {
			case TYPE_PSTRING:
				writePString((String) object);
				break;
			case TYPE_AFPSTRING:
				writeAFPString((String) object);
				break;
			case TYPE_PSTRING_ARR:
				writePStringArray((String[]) object);
				break;
			case TYPE_BYTE_ARR:
				writeBytes((byte[]) object);
				break;
			default:
				error("unknown deferred (" + deferredOffset + ") object type: " + type);
				break;
			}
		}
